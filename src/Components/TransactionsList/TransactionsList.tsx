...
const parseDate = (dateStr: string) => {
    ...
    return new Date(2026, new Date(`${month} 1 2026`).getMonth(), parseInt(day));
};
...
const TransactionsList = () => {
    ...
    const toggleRole = () => setRole(role === 'Admin' ? 'User' : 'Admin');
    ...
    return (
        <motion.section
            ...
            className="min-h-screen bg-[#faf8ff] rounded shadow-xl px-4 lg:px-10 py-3 pb-20 lg:pb-10 overflow-y-auto">
            <Navbar />

            <div className={`mt-4 flex items-center gap-2 p-3 px-6 text-xs lg:text-sm rounded-xl border transition-all duration-300 ${role === 'Admin'
            ...
            <div className="flex flex-col lg:flex-row gap-6 pt-8">
                            ...
                            {(['All', 'Income', 'Expense'] as const).map((type) => (
                                <button
                                    ...
                                    className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${filterType === type
                                        ...
                                        : 'text-black/50 hover:text-white hover:bg-primary'
                                        }`}
                                >
                ...
                <div className="lg:w-80 bg-linear-to-br from-[#0047AB] to-[#003177] p-6 rounded-2xl shadow-xl shadow-primary/20 flex flex-col justify-center">
                    ...
                    <p className="text-white/30 text-xs font-bold mt-2">{filteredTransactions.length} transaction{filteredTransactions.length !== 1 ? 's' : ''}</p>
                </div>
            </div>
                    ...
                    {filteredTransactions.length > 0 ? (
                        filteredTransactions.map((tx) => (
                            <motion.div
                                key={tx.id}
                                variants={item}
                                className="bg-white p-5 rounded-2xl border border-black/5 shadow-sm flex items-center justify-between"
                            >
                                <div className="flex items-center gap-4">
